...
module.exports = function(options) {
  ...
  function listKeys(req, res, next) {
    ...
    s3.listObjectsV2(s3Params, function(err, data) {
      ...
      console.log('s3 prefix: ' + s3Params.prefix);

      var keys = [];
      data.CommonPrefixes.forEach(function(prefix) {
        console.log('processing prefix ' + prefix);

        if (prefix !== s3Params.prefix) {
          if (isEmpty(s3Params.prefix)) {
            console.log('pushing prefix ' + prefix.Prefix);

            keys.push(prefix.Prefix);
          } else {
            console.log('pushing partial prefix ' + prefix.Prefix.substr(s3Params.prefix.length));

            keys.push(prefix.Prefix.substr(s3Params.prefix.length));
          }
        }
      });
